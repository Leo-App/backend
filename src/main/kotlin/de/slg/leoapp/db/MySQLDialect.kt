...
import tel.egram.kuery.sqlite.SQLiteDefinition
...
object MySQLDialect : Dialect {
    ...
    fun <T : Table> build(statement: BatchInsertStatement<T>): String {
        ...
        for (i in 0 until statement.assignments.size) {
            ...
            for (assign in statement.assignments) {
                ...
                if (i >= assign.values.size) {
                    appendValue(builder, assign.values.size - 1)
                } else {
                    appendValue(builder, assign.values[i])
                }
            }
    ...
    fun <T : Table> build(statement: UpsertStatement<T>): String {
        val builder = StringBuilder()
        builder.append("INSERT INTO ")
        appendTableName(builder, statement.subject.table)
        builder.append(" (")
        ...
        for (assign in statement.upsert.assignments) {
            builder.append(delim)
            delim = ", "
        ...
        delim = ""
        for (cur in statement.upsert.updates) {
            builder.append(delim)
            delim = ", "

            appendPredicate(builder, EqExpression(cur.column, cur.value))
        }
    ...
    private fun appendPredicate(builder: StringBuilder, value: Any?, fullFormat: Boolean = true) {
        when (value) {
            ...
            is InExpression -> {
                ...
                appendPredicate(builder, value.left, fullFormat)
                builder.append(" IN ")
                appendPredicate(builder, getINListRepresentation(value.right), fullFormat)
                builder.append(')')
            }
    ...
    private fun appendProjection(builder: StringBuilder, projection: Iterable<Projection>, fullFormat: Boolean) {
        if ("SELECT".contentEquals(builder.toString().trim()) and projection.none()) {
            ...
            for (proj in projection) {
                ...
                } else if (proj is CountProjection) {
                    builder.append("COUNT(")
                    if (fullFormat) {
                        appendFullColumnName(builder, proj.column)
                    } else {
                        appendShortColumnName(builder, proj.column)
                    }
                    builder.append(") as c_${proj.column}")
                } else {
                    builder.append(proj)
                }
            }
    ...
    private fun getINListRepresentation(list: List<Any?>) = "(${list.joinToString()})"
